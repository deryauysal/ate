...
// Version: 12/27/2022
// 
//Function for the asmptotic variance of  the ipw1 in Uysal (2022)
//Input: 
// Data & specifications
//     struct DS ds_struct;
//     ds_struct = dsCreate;
...
proc(1)= asyvar_ipw1(teta, struct DS ds_struct); 
...
psi1 = (indep.*(treat-psest)); //Prob. function logit.
    Emu1alp=-meanc(((treat.*dep)./(psest.^2)).*psest.*(1-psest).*indep);
    Emu0alp=meanc((((1-treat).*dep)./((1-psest).^2)).*psest.*(1-psest).*indep);
...
psi1 = (indep.*dpsest.*(treat-psest)./(psest.*(1-psest))); 
    Emu1alp=-meanc(((treat.*dep)./(psest.^2)).*dpsest.*indep);
    Emu0alp=meanc((((1-treat).*dep)./((1-psest).^2)).*dpsest.*indep);
endif;
EH = -(1/N)*psi1'*psi1;
Vp = meanc((treat.*dep.^2)./psest.^2+((1-treat).*dep.^2)./(1-psest).^2)-(mu1-mu0)^2;
